...
export const defaultOptions: Readonly<NodeHtmlMarkdownOptions> = Object.freeze({
   ...
   */
  lineStartEscape: [/^(\s*?)((?:\+\s)|(?:[=>-])|(?:#{1,6}\s))|(?:(\d+)(\.\s))/gm, "$1$3\\$2$4"] as const,
...
export const defaultTranslators: TranslatorConfigObject = {
  ...
  "del,s,strike": {
    ...
    postprocess: ({ content, options: { strikeDelimiter } }) =>
      isWhiteSpaceOnly(content) ? PostProcessResult.RemoveNode : tagSurround(content, strikeDelimiter),
  },
  ...
  "em,i": {
    ...
    postprocess: ({ content, options: { emDelimiter } }) =>
      isWhiteSpaceOnly(content) ? PostProcessResult.RemoveNode : tagSurround(content, emDelimiter),
  },
    ...
    postprocess: ({ content, nodeMetadata, node }) => {
      ...
      for (const row of rawRows) {
        ...
        const cols = row.split(" |").map((c, i) => {
          c = c.trim();
          if (colWidth.length < i + 1 || (colWidth[i] && colWidth[i] < c.length)) colWidth[i] = c.length;
