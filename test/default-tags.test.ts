...
describe(`Default Tags`, () => {
  ...
  test(`Bold (b, strong)`, () => {
    ...
    const exp = `**a~~b~~** \n  \n**c** \n**d**`;
    strictEqual(res, exp + " " + exp);
  });

  test(`Strikethrough (del, s, strike)`, () => {
    const res = translate(
      `<del>a<em>b</em><br><br>c<br>d</del><s>a<em>b</em><br><br>c<br>d</s><strike>a<em>b</em><br><br>c<br>d</strike>`
    );
    const exp = `~~a_b_~~ \n  \n~~c~~ \n~~d~~`;
    strictEqual(res, exp + " " + exp + " " + exp);
  });
  ...
  test(`Link (a)`, () => {
    ...
    strictEqual(
      res,
      `[a b**c**](${url}) a**b** <${url}> [a](${url})[nested](2)![](${url})b **_[b](${encodedSpecialUrl} "a")_** `
    );
  });
  ...
  test(`Image (img)`, () => {
    ...
    `);
    strictEqual(res, `![](${url}1)` + ` ![](${url}3 "t3")` + ` ![a4](${url}4 "t4") `);
  });
  ...
  test(`Headings (h1, h2, h3, h4, h5, h6)`, () => {
    ...
    strictEqual(
      res,
      Array.from(Array(6), (v, i) => `#`.repeat(i + 1) + ` a**b**\n\n`)
        .join("")
        .trim()
    );
  });
